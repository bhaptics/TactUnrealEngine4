	...
	public:
		...
		void poll(int timeout = 0) { // timeout in milliseconds
			...
			if (timeout != 0) {
				...
				select((int)(sockfd + 1), &rfds, &wfds, 0, timeout > 0 ? &tv : 0);
			}
			while (true) {
